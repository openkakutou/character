...
| HeaderExprs | map[string]string | Raw, unevaluated source text (keyed by lowercase field name) of `anim`/`poweradd`/`juggle`/`sprpriority` whenever the source held a MUGEN trigger expression rather than a literal integer, and of `ctrl`/`facep2`/`hitdefpersist`/`movehitpersist`/`hitcountpersist` whenever the source held one rather than a literal bool; the corresponding typed field stays at its zero value in either case |
