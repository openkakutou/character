...
## Acceptance Criteria
- [x] Parsing an empty file returns a clear error (or an explicit empty result, per the chosen contract) rather than panicking
- [x] A malformed action header or frame line returns an error identifying the offending line
- [x] Comment lines (`;`) are ignored and do not break parsing of surrounding valid lines
- [x] Negative group/image indices are rejected with a descriptive error
