...
## Acceptance Criteria
- [x] `.cmd` files parse into a structured `Command` model (name, input sequence, buffer time)
- [x] Round-trip serialize preserves original formatting for unmodified content, same guarantee as other formats
- [x] Both MUGEN-style and Ikemen GO-style `.cmd` syntax variants parse correctly (fixture-driven against real files, same practice as `.cns`)
- [x] A malformed `.cmd` file returns a descriptive error instead of crashing
