...
## Acceptance Criteria
- [x] Parsing a valid multi-action `.air` sample produces `Animation` structs with correct action numbers, frame sequences, and Clsn boxes
- [x] `Loopstart` is correctly recorded so looping animations are distinguishable from non-looping ones
- [x] Frame optional fields (flip, blend mode) default correctly when omitted from a line
