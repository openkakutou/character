...
## Acceptance Criteria
- [x] `sff/` package removed from this repo; `go.mod` depends on `github.com/openkakutou/sff` instead
- [x] All existing behavior (JSON contract, WASM exports) is unchanged from a consumer's point of view — no breaking change to `character-viewer-web` or the published WASM API
- [x] All existing tests that previously exercised the internal `sff` package now pass against the external module without modification to their assertions
- [x] A malformed/incompatible `sff` module version is caught at build time (`go.mod` version pin), not silently
