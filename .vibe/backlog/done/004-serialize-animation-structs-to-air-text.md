...
## Acceptance Criteria
- [x] Serializing a multi-action `Animation` produces valid `.air` text that re-parses (via item 002's parser) into an equivalent structure
- [x] Clsn boxes and `Loopstart` are correctly emitted
- [x] Output uses consistent, valid MUGEN/Ikemen `.air` syntax (verified by re-parsing it)
