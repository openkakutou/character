...
## Acceptance Criteria
- [x] `cmd/wasm/main.go` builds successfully with `GOOS=js GOARCH=wasm go build`
- [x] The WASM module exposes JS-callable functions to load a character (from `.def` + its referenced files) and read back its animations/sprites/metadata, without any canvas/OpenGL/rendering code
- [x] A tagged release triggers a GitHub Actions workflow that builds the wasm and publishes `.wasm` + `wasm_exec.js` as downloadable release assets
- [x] A consumer can download a specific version's release assets and load the module in a browser (documented with a minimal usage snippet in the README or `docs/`)
