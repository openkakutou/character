...
## Acceptance Criteria
- [ ] A `.def`-referenced file path containing backslash separators (e.g. `"states\constants.cns"`) resolves to the correct nested file on a forward-slash filesystem
- [ ] A `.def`-referenced file path using forward slashes, or no separator at all, resolves exactly as today (no regression)
- [ ] `~/workspace/ikemen-quick-versus/chars/Dragon Ball/Bardock/Bardock.def` (references `bardock_unotag\bardock.cns`) loads successfully after the fix
- [ ] `~/workspace/ikemen-quick-versus/chars/Nintendo/Yoshi/Yoshi.def` (references `graphics\yoshi.air`) loads successfully after the fix
- [ ] `~/workspace/ikemen-quick-versus/chars/DC Comics/Black Manta/Black Manta.def` (references `BLACK MANTA_unotag\BLACK MANTA.cns`) loads successfully after the fix
