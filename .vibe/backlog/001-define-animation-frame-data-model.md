...
## Acceptance Criteria
- [ ] `Animation`, `Frame`, and `ClsnBox` structs are exported from the `air` package with documented fields matching the `.air` format (group, image, x, y, time, flip, blend mode; Clsn box coordinates)
- [ ] Zero-value `Animation{}` and `Frame{}` compile and behave predictably (no nil-pointer panics), covered by a zero-value test following the `character_test.go` pattern
- [ ] No parsing, file I/O, or write-only logic present in this package yet
