...
## Acceptance Criteria
- [ ] `air.Parse` accepts a frame line where `Group` and/or `Image` is any negative value (not just exactly `-1`) as the blank-frame sentinel, matching `Frame.IsBlank()`
- [ ] A frame parsed from such a line resolves via `SpriteResolver`/`character.ResolveSprite` the same way an existing `-1,-1` blank frame does (no sprite lookup attempted)
- [ ] Existing behavior for a genuinely malformed frame line (non-numeric fields, too few fields) is unchanged — still a descriptive, line-numbered error
- [ ] `.vibe/decisions/013-blank-frame-sentinel-representation.md` is superseded by a new ADR recording the corrected rule (its `status` updated to `superseded by NNN`, per the ADR append-only convention)
