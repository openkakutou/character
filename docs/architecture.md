...
```mermaid
graph TD
    root["character (root package)<br/>Character struct<br/>+ ResolveSprite(frame)<br/>+ Load(defPath)<br/>+ SerializeDef/Air/Cns/Cmd/Zss"]
    ...
    root -->|saves via Document/Serialize of| air
    root -->|saves via Document/Serialize of| cns
    root -->|saves via Document/Serialize of| cmd
    root -->|saves via Document/Serialize of| zss
```
