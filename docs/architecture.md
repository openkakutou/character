...
graph TD
    ...
    def["character/def<br/>CharacterInfo (data model)<br/>+ Parse(.def text)"]
...
| `character/def` | Character definition (`.def`) files — the entry point referencing the other formats: the `CharacterInfo` data model and a text parser (`Parse`) | Data model and read path implemented (`CharacterInfo`; `Parse` reads `[Info]`/`[Files]` text into it, skipping unrecognized sections); serialization and wiring into the root `Character` struct not yet implemented |
...
alongside comments and original section ordering — is deferred to the write
path (backlog item 017), mirroring `air`'s `Document` split above. See
