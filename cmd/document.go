...
// .cmd file can be round-tripped — parsed, then serialized back out —
// without losing the comments, section ordering, and unrecognized sections
// that the pure-data CommandFile model deliberately does not carry.
//
// Document.File is decoded the same way Parse's return value is, for
// convenient structured access to what was parsed — but Serialize does not
// read it back. As long as Document.File is left untouched, ParseDocument
// followed by Serialize reproduces the original source byte-for-byte,
// comments and all. Mutating File has no effect on Serialize's output:
// regenerating text from an edited CommandFile while still preserving
...
// ParseDocument reads MUGEN/Ikemen GO .cmd input-command text from r,
...
func ParseDocument(r io.Reader) (*Document, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cmd: reading document source: %w", err)
	}
	...
	return &Document{File: file, source: source}, nil
}
...
func (d *Document) Serialize(w io.Writer) error {
	if _, err := w.Write(d.source); err != nil {
		return fmt.Errorf("cmd: writing document: %w", err)
	}
	return nil
}
