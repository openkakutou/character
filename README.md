...
- Reading real-world `.air` files that use Ikemen GO's animation interpolation markers (smoothly transitioning a property across an animation) instead of rejecting them
...
- Reading MUGEN/Ikemen GO character definition (`.def`) files into that data model: name, author, and every referenced file (sprite, animation, sound, commands, combat logic, additional states, palettes); sections the library doesn't recognize are skipped instead of aborting the read, and a malformed section header is caught with a clear, line-numbered error instead of crashing
...
- Reading real-world `.def` files that contain a stray line inside a recognized section (a leftover truncated key, a decorative separator) instead of rejecting them — that line is simply skipped, the same way an unrecognized setting already is
