...
// pixel buffer.
//
// For indexed pixel data (V2FormatRaw, V2FormatPNG8), BytesPerPixel is 1
// and each byte in Pixels is a palette index — no RGB/palette resolution
// performed, mirroring PCXImage. For direct-color pixel data (V2FormatPNG24,
// V2FormatPNG32), BytesPerPixel is 3 (RGB) or 4 (RGBA, straight alpha) and
// each pixel's actual color channels are stored directly, since no palette
// is involved for those formats.
type V2Image struct {
...
//
// Only V2FormatRaw (uncompressed indexed data) and the PNG formats
// (V2FormatPNG8/24/32, decoded via the standard library) are supported.
// Any other format code — including the real but unimplemented compressed
// formats V2FormatRLE8/RLE5/LZ5 — returns a descriptive error rather than
// panicking or silently misinterpreting the data; see
